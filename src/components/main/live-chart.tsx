...
import type { ActiveBooster, DataPoint } from "@/lib/constants";
...
export function LiveChart() {
	const { data: chartData } = useSWR<DataPoint[]>(
		`${baseURL}/api/bot-data?type=mev&count=50`,
		fetcher,
	);
	...
	const { data: activeBoosters } = useQuery<ActiveBooster[]>({
		queryKey: ["active-boosters"],
		queryFn: async () => fetcher(`${baseURL}/api/boosters/active`),
	});
